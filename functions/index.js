...
exports.login = onRequest(async (request, response) => {
    ...
    if (Body.w3w[0] === respo["savedW3W"][0] && Body.w3w[1] === respo["savedW3W"][1] && Body.w3w[2] === respo["savedW3W"][2]) {
        if (getDistFromLatLonInKm(Body.lat, Body.long, respo.savedLocation[0], respo.savedLocation[1]) <= 0.5) {
            cod.update(
                {
                    savedAttemptes: {
                        ...(await cod.get()).data()["savedAttemptes"],
